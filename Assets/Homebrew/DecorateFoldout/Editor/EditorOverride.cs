...
namespace Pixeye.Unity
{
	...
	public class EditorOverride : Editor
	{
		...
		void Child(int i,CacheFoldProp cache)
		{
		...
		void Foldout(CacheFoldProp cache)
		{
			...
			if (cache.expanded)
			{
				...
				{
					this.UseVerticalLayout(() => Child(i,cache), StyleFramework.boxChild);
				}
			}
		}
		...
		void Setup()
		{
			...
			if (!initialized)
			{
				...
				//FoldoutAttribute prevFold = ;
				FoldoutAttribute prevFold = new FoldoutAttribute("",false);
	...
	static class StyleFramework
	{
		...
		static StyleFramework()
		{
			...
			box.padding = new RectOffset(5, 0, 10, 0);
