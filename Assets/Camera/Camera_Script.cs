...
public sealed partial class Camera_Script : MonoBehaviour
{
    #region 前回カメラ
#if false
    private GameObject enemy;                // 敵のオブジェクトを取得
    public static bool enemy_hit_flg = false;
    ...
    void OnGUI()
    {
		if (gui_on) {
            ...
            GUILayout.TextArea("Enemy.shot_touch_flg\n" + Enemy.shot_touch_flg);
            GUILayout.TextArea("enemy_hit_flg\n" + enemy_hit_flg);//save_pos
...
#if true
    //private GameObject[] enemy;                // 敵のオブジェクトを取得
    //private const int ENM_NUM = 4;
    //private Vector3[] enm_pos;
    public static bool enemy_hit_flg = false;
    private Vector3 enm_pos;
...
void Update()
    {
        //if (Enemy.shot_touch_flg)
        //{
        //    enm_pos = obj.transform.position;
        //    enemy_hit_flg = true;
        //}
        ...
        for (int i = 0; i < obj.Length; i++)
        {
            ...
            {
                enemy_hit_flg = true;
                enm_pos = obj[i].transform.position;
            }
        }
...
#region 注視点設定
    Vector3 player_target() { return new Vector3(player.transform.position.x, player.transform.position.y + UP_TARGET, player.transform.position.z); }
    Vector3 enemy_target(Vector3 look_pos)
    {
        return Vector3.Lerp(transform.position + look_pos, enm_pos, LOOK_SPD * Time.deltaTime);
    }
    Vector3 world_target() { return Vector3.zero; }
